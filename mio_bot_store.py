...
# --- DATI E PREZZI ---
CHANNELS = {
    "goal": "Football Goal ⚽️", "juve": "Juventus Planet ⚪️⚫️",
...
INCREMENT_PACKAGES = {
    "1K": "1K Membri - 50€", "2K": "2K Membri - 80€",
    "3K": "3K Membri - 120€", "5K": "5K Membri - 200€"
}
...
async def main_menu(obj):
    ...
    txt = "👋 **Benvenuto nel Global Advertising Bot!** Scegli un'opzione:"
...
async def render_channels(callback, sel):
    ...
    await callback.message.edit_text("📣 **Seleziona i canali per la sponsorizzazione:**", reply_markup=kb.as_markup())

@dp.callback_query(F.data.startswith("ch_"))
async def handle_channels(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data(); sel = data.get('channels', []); c = callback.data.replace("ch_", "")
    if c == "all": sel = list(CHANNELS.keys())
    elif c == "none": sel = []
    elif c in sel: sel.remove(c)
    else: sel.append(c)
    await state.update_data(channels=sel); await render_channels(callback, sel)

@dp.callback_query(F.data == "go_dur")
async def go_dur(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if not data.get('channels', []):
        await callback.answer("Devi selezionare almeno un canale!", show_alert=True); return
    kb = InlineKeyboardBuilder()
    ...
    await callback.message.edit_text("⏳ **Scegli la durata della sponsorizzazione:**", reply_markup=kb.as_markup())
...
async def custom_dur_prompt(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✍️ Scrivi il numero di ore desiderate (Min 3, Max 24):", reply_markup=kb.as_markup())
...
async def custom_dur_input(message: types.Message, state: FSMContext):
    try:
        ...
        await state.update_data(duration=h); await render_extras(message, state)
    except: await message.answer("⚠️ Inserisci un numero valido tra 3 e 24.")

@dp.callback_query(Flow.duration, F.data.startswith("dur_"))
async def handle_dur(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(duration=int(callback.data.replace("dur_", ""))); await render_extras(callback, state)

async def render_extras(obj, state):
    data = await state.get_data(); rep = data.get('ext_repost', False); fiss = data.get('ext_fiss', False); nop = data.get('ext_nopost', 0)
    kb = InlineKeyboardBuilder()
    ...
    txt = "✨ **Aggiunte alla tua sponsorizzazione:**"
    if isinstance(obj, types.Message): await obj.answer(txt, reply_markup=kb.as_markup())
    else: await obj.message.edit_text(txt, reply_markup=kb.as_markup())
...
async def handle_extras(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data(); v = callback.data.replace("ex_", "")
...
async def render_calendar(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data(); page = data.get('cal_page', 0)
    kb = InlineKeyboardBuilder(); start_idx = page * 9
    ...
    await callback.message.edit_text("📅 **Scegli la data di pubblicazione:**", reply_markup=kb.as_markup())
...
async def render_times(callback: types.CallbackQuery, state: FSMContext):
    sel_date = callback.data.replace("dt_", ""); await state.update_data(date=sel_date)
    data = await state.get_data(); dur_h = data.get('duration')
    intervals = get_booked_intervals(); year = datetime.now().year
    ...
    kb = InlineKeyboardBuilder(); times = ["09:00", "12:00", "15:00", "18:00", "21:00", "23:00"]
    for t in times:
        try:
            start_dt = datetime.strptime(f"{sel_date}/{year} {t}", "%d/%m/%Y %H:%M")
            if start_dt < datetime.now() - timedelta(days=60): start_dt = start_dt.replace(year=year+1)
            end_dt = start_dt + timedelta(hours=dur_h)
            overlap = False
            for b_start, b_end in intervals:
                if start_dt < b_end and end_dt > b_start:
                    overlap = True; break
            if not overlap: kb.add(types.InlineKeyboardButton(text=t, callback_data=f"tm_{t}"))
        except: pass
    kb.adjust(2)
    ...
    txt = (f"📅 **Disponibilità per il {sel_date}:**\n{busy_text}\n\n"
           f"⏰ **Scegli un orario di inizio** (Durata: {dur_h}h):")
...
async def custom_time_prompt(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✍️ Scrivi l'orario nel formato HH:MM (es. 14:30):", reply_markup=kb.as_markup())
...
async def custom_time_input(message: types.Message, state: FSMContext):
    try:
        t_str = message.text.strip(); datetime.strptime(t_str, "%H:%M")
        data = await state.get_data(); sel_date = data.get('date'); dur_h = data.get('duration')
        intervals = get_booked_intervals(); year = datetime.now().year
        ...
        for b_start, b_end in intervals:
            if start_dt < b_end and end_dt > b_start:
                overlap_info = f"{b_start.strftime('%H:%M')} e le {b_end.strftime('%H:%M')}"; break
        if overlap_info:
            await message.answer(f"⚠️ Non puoi iniziare alle {t_str} perché c'è già una sponsor tra le {overlap_info}.\n\nProva un altro orario:"); return
        await state.update_data(time=t_str); await render_recap(message, state)
    except: await message.answer("⚠️ Formato non valido. Usa HH:MM.")

@dp.callback_query(Flow.time, F.data.startswith("tm_"))
async def handle_time(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(time=callback.data.replace("tm_", "")); await render_recap(callback, state)

async def render_recap(obj, state):
    data = await state.get_data(); h = data['duration']; base_price = calculate_price(data['channels'], h)
    ...
    start_t = data['time']; end_dt = datetime.strptime(start_t, "%H:%M") + timedelta(hours=h)
    end_t = end_dt.strftime("%H:%M"); await state.update_data(tot=tot, end_t=end_t)
    ...
    recap = (f"🛒 **IL TUO CARRELLO**\n\n📦 Pacchetto: Sponsor\n📺 Canali: {', '.join(ch_names)}\n⏳ Ore: {h}h\n📅 Data: {data['date']}\n⏰ Inizio: {start_t} | Fine: {end_t}\n✨ Aggiunte: {extra_str}\n\nTOTALE: {tot:.2f}€")
    kb = InlineKeyboardBuilder()
    ...
    else: await obj.message.edit_text(recap, reply_markup=kb.as_markup())

@dp.callback_query(F.data == "pay_sponsor")
async def pay_sponsor(callback: types.CallbackQuery, state: FSMContext):
    cau = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6)); await state.update_data(causale=cau)
    txt = (f"💶 **PROCEDI CON IL PAGAMENTO**\n\nIBAN: `{IBAN_DATI}`\nCausale: `ADV-{cau}`\n\n📸 Invia qui sotto lo screenshot del pagamento.")
    await callback.message.edit_text(txt, reply_markup=InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="❌ Annulla", callback_data="back_main")).as_markup()); await state.set_state(Flow.receipt_sponsor)
...
async def rx_sponsor(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect('ads_booking.db')
    conn.execute("INSERT INTO bookings (user_id, type, info, date, start_t, end_t, causale, status) VALUES (?,?,?,?,?,?,?,?)", (u.id, "Sponsor", f"{len(d['channels'])} Canali, {d['duration']}h", d['date'], d['time'], d['end_t'], d['causale'], "IN ATTESA"))
    bid = conn.execute("SELECT last_insert_rowid()").fetchone()[0]; conn.commit(); conn.close()
    ...
    await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=admin_txt, reply_markup=kb.as_markup()); await message.answer("✅ Ricevuta inviata! In attesa di approvazione."); await state.clear()

# --- ALTRE FUNZIONI (CONTROLLO ORDINI, ADMIN, INCREMENTI) ---
...
async def order_status(callback: types.CallbackQuery):
    ...
    txt = "🔍 **I TUOI ULTIMI ORDINI:**\n\n" + "\n\n".join([f"📦 {r[0]} ({r[1]})\n📅 {r[2]} ore {r[3]}\nStato: {r[4]}" for r in rows]) if rows else "🔍 Nessun ordine."
...
async def how_works(callback: types.CallbackQuery):
    await callback.message.edit_text("⚙️ **Come Funziona**\n1.Scegli Sponsor o Incrementi\n2.Scegli orario/data\n3.Paga via IBAN e invia screen\n4.Attendi approvazione!", reply_markup=InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="⬅️ Indietro", callback_data="back_main")).as_markup())

@dp.callback_query(F.data.startswith("adm_ok_"))
async def adm_ok(callback: types.CallbackQuery):
    bid = int(callback.data.replace("adm_ok_", "")); conn = sqlite3.connect('ads_booking.db')
    conn.execute("UPDATE bookings SET status = 'APPROVATO' WHERE id = ?", (bid,))
    uid = conn.execute("SELECT user_id FROM bookings WHERE id = ?", (bid,)).fetchone()[0]; conn.commit(); conn.close()
...
async def adm_no(callback: types.CallbackQuery):
    bid = int(callback.data.replace("adm_no_", "")); conn = sqlite3.connect('ads_booking.db')
    conn.execute("UPDATE bookings SET status = 'RIFIUTATO' WHERE id = ?", (bid,))
    uid = conn.execute("SELECT user_id FROM bookings WHERE id = ?", (bid,)).fetchone()[0]; conn.commit(); conn.close()
    await bot.send_message(uid, "❌ **IL TUO ORDINE È STATO RIFIUTATO.**"); await callback.message.edit_caption(caption=callback.message.caption + "\n\n🔴 STATO: RIFIUTATO")

# --- INCREMENTI (VERSIONE RAPIDA) ---
...
async def buy_increment(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📈 **Scegli il pacchetto Incrementi:**", reply_markup=kb.as_markup()); await state.set_state(Flow.inc_package)
...
async def inc_package_sel(callback: types.CallbackQuery, state: FSMContext):
    pkg_id = callback.data.replace("inc_", ""); await state.update_data(inc_pkg=pkg_id, inc_name=INCREMENT_PACKAGES[pkg_id])
    await callback.message.edit_text("🔗 Invia il link del canale/gruppo:", reply_markup=InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="❌ Annulla", callback_data="back_main")).as_markup()); await state.set_state(Flow.inc_link)

@dp.message(Flow.inc_link)
async def inc_link_rx(message: types.Message, state: FSMContext):
    await state.update_data(inc_link=message.text); cau = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6)); await state.update_data(causale=cau)
    await message.answer(f"💶 **PAGAMENTO**\nIBAN: `{IBAN_DATI}`\nCausale: `INC-{cau}`\n\nInvia lo screen:"); await state.set_state(Flow.inc_receipt)

@dp.message(Flow.inc_receipt, F.photo)
async def rx_inc(message: types.Message, state: FSMContext):
    d = await state.get_data(); u = message.from_user
    admin_txt = f"🆕 **INCREMENTO!**\n👤 @{u.username}\n📦 {d['inc_name']}\n🔗 {d['inc_link']}\n🔑 INC-{d['causale']}"
    conn = sqlite3.connect('ads_booking.db')
    conn.execute("INSERT INTO bookings (user_id, type, info, date, start_t, end_t, causale, status) VALUES (?,?,?,?,?,?,?,?)", (u.id, "Incremento", d['inc_name'], "N/D", "N/D", "N/D", d['causale'], "IN ATTESA"))
    bid = conn.execute("SELECT last_insert_rowid()").fetchone()[0]; conn.commit(); conn.close()
    kb = InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="✅ APPROVA", callback_data=f"adm_ok_{bid}"), types.InlineKeyboardButton(text="❌ RIFIUTA", callback_data=f"adm_no_{bid}"))
    await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=admin_txt, reply_markup=kb.as_markup()); await message.answer("✅ Ricevuta inviata!"); await state.clear()
